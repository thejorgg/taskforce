...
- Stage cards, legend entries, approval buttons, and run rows are clickable; the mouse wheel scrolls the spy view.
...
Built-in adapters: `claude`, `codex`, `opencode`, and `gemini`. Each has a plan variant (read-only) and a build variant (allowed to edit the workspace). Set `agent` and optionally `model` on `relay.control` / `relay.build` to pick one.
...
From the dashboard settings screen (`ctrl+p`), edit config with commands such as:
